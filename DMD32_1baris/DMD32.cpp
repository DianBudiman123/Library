...
void DMD::scanDisplayBySPI()
{
   ...
   //if( digitalRead( PIN_OTHER_SPI_nCS ) == HIGH )
   {
		...
		vspi->begin(PIN_DMD_CLK, 23, PIN_DMD_R_DATA, 23);
		...
		vspi->end();
		 ...
		 //offset=rowsize * bDMDByte;
		
       
		vspi->begin(PIN_DMD_CLK2, 23, PIN_DMD_R_DATA, 23);
		for (int i=(rowsize/4);i<(rowsize);i++) {
			vspi->beginTransaction(SPISettings(spiClk, MSBFIRST, SPI_MODE0));
			vspi->transfer(bDMDScreenRAM[offset+i+row3]);
			vspi->transfer(bDMDScreenRAM[offset+i+row2]);
			vspi->transfer(bDMDScreenRAM[offset+i+row1]);
...
boolean DMD::stepSplitMarquee3(int topRow, int bottomRow,int StartColumn)
{
  boolean ret=false;
